...
import { KMeans } from "@jrc03c/js-data-science-helpers"
import { pause } from "@jrc03c/pause"
import { sse } from "../metrics.mjs"
import { stack, tensor, tidy } from "@tensorflow/tfjs"

const { KMeansNaive } = KMeans
...
class TFKMeansNaive extends KMeansNaive {
  initializeCentroids(x) {
    ...
    return tensor(super.initializeCentroids(float(x)))
  }
  ...
  getFitStepFunction(x, progress) {
    ...
    const xtf = addTensorToScope(tensor(x))
    ...
    return async () => {
      try {
        ...
        temp = addTensorToScope(stack(temp))
  ...
  predict(x, centroids) {
    ...
    return tidy(() => {
      ...
      const xtf = tensor(x).expandDims(1)
