...
test("tests that the `TFKMeansPlusPlus` model works correctly", async () => {
  ...
  const model2 = new TFKMeansPlusPlus({ k: centroidsTrue.length })
...
test("tests that the `TFKMeansPlusPlus` model can handle BigInts", async () => {
  ...
  const model = new TFKMeansPlusPlus({ k: centroids.length })
  await model.fit(x)
